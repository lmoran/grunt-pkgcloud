...
var pkgcloud = require("pkgcloud"), _ = require("underscore"), grunt = require("grunt"), exec = require('child_process').exec;
...
/**
 * Prints on the Grunt log the primitive data types of an Object
 * 
 * @gparam {Object} grunt Grunt instance
 * @param {Object}
 *          result Result object to be logged
 * @param {Function}
 *          done Callback
 */
utils.logResultObject = function(grunt, result, done) {
  grunt.log.ok(utils.primitiveAttributes(result));
  grunt.log.ok(utils.primitiveValues(result));
  done();
};

/**
 * Executes a Grunt task (used during integration tests)
 * 
 * @param {String}
 *          taks Name of Grunt task
 * @param {Object}
 *          options Grunt options to pass
 */
utils.gruntExec = function(task, options, callback) {
  var cliOptions = "";
  _.keys(options).forEach(function(opt) {
    if (options[opt]) {
      cliOptions += " --" + opt + "=\"" + options[opt] + "\"";
    }
  });
//  console.log("GRUNTEXEC: " + "grunt pkgcloud:" + task + " " + cliOptions); // XXX
  exec("grunt pkgcloud:" + task + " " + cliOptions, callback);
};

/**
 * Returns the test configuration loaded from "config.json"
 * 
 * @return {Object}
 */
utils.getConfig = function() {
  return JSON.parse(require("fs").readFileSync("./config.json", "utf8"));
};
